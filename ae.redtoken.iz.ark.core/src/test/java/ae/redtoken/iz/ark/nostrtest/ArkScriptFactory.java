...
public class ArkScriptFactory {
    ...
    public static TransactionWitness createVTXONodeUnlockWitnessScript(List<byte[]> userSignatures, byte[] program) {

        List<byte[]> pushes = new ArrayList<>(userSignatures);
        pushes.add(new byte[]{0x01});
        pushes.add(program);
    ...
    public List<byte[]> extractPrimaryUnlockPublicKeysFromVTXO(Script unlockScript) {
        ...
        for (int i = 0; unlockScript.chunks().get(i).opcode != ScriptOpCodes.OP_CHECKSIG; i += 2) {
            userSignatures.add(unlockScript.chunks().get(i + 1).data);
        }
